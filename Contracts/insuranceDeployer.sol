...
contract InsuranceDeployer {
  ...
  constructor(LinkTokenInterface _link, address _registrar, AutomationRegistryInterface _registry) {
   ...
   // Function to receive Ether. msg.data must be empty
    ...
    function createInsurance(int _thresh, ISuperToken _token, address assetInsured, uint _insuredAmt, address _tokenAddress, int96 _flowRate) public {
        IERC20 tokenAsset = IERC20(_tokenAddress);
        // tokenAsset.transferFrom(msg.sender, address(this), _insuredAmt);
        address insurance = address(new InsuranceBond(_thresh,  _token,  assetInsured,  _insuredAmt,  _tokenAddress,  _flowRate,msg.sender));
        InsurancesRegistered.push(insurance);
        totalInsuranceBonds++;
        uint96 amount=1000000000000000000;
        bytes memory dummy = abi.encodePacked("0x");
        insuranceName=string(abi.encodePacked("insurance", totalInsuranceBonds));
        address payable insurancePayable = payable(insurance);
        transferEther(insurancePayable);
        tokenAsset.transfer(insurance, _insuredAmt);
        registerAndPredictID(insuranceName,dummy,insurance,500000,owner,dummy,dummy,amount);
    }
        function transferEther(address payable _recipient) public payable {
        _recipient.transfer(0.1 ether);
    }
